...
function f_delete(){
	...
	if(count==0){
		...
		formm.action="/wepapp/NonageServlet2?command=in_delete&free=admin"
		formm.submit();
	}
}
function go_search(){
	formm.action="NonageServlet2?command=admin_inquiry";
	formm.submit();
}
function go_total(){
	formm.action="NonageServlet2?command=admin_inquiry";
...
<%@ include file="Aheader.jsp" %>
	...
	<table>
		...
		<table class="table table-striped">
			...
			<tr>
			<th>${item.qseq}</th>
			<th><a href="NonageServlet2?command=admin_QnaDetail&qseq=${item.qseq}">${item.subject}</a></th>
			<th>${item.id}</th>
			<th>
